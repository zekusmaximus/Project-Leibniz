...
//
// The starter story graph — pure data, no dependencies. Split out of seed.js so
// it can be imported both by the seeding script (which writes it to MongoDB) and
// by tests (which compare it against the client's bundled InitialState.ts) WITHOUT
// pulling in mongoose or opening a database connection.
//
// Mirrors the bundled client fallback in client/src/context/InitialState.ts.
// Choice/textVariant `condition` values are serialized ConditionSpec strings (see
// client/src/services/conditionDSL.ts); the client compiles them back into
// predicates in storyMapper.ts, so a condition behaves identically whether the
// graph is served from here or loaded from the offline fallback. Keep these specs
// in sync with the compiled specs in InitialState.ts — `tests/seedSync.test.ts`
